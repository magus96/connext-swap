 ...
 *
 */
class SdkUtils extends sdkShared_1.SdkShared {
    ...
    static async create(_config, _logger, _chainData) {
        ...
        const logger = _logger
            ...
            : new nxtp_utils_1.Logger({ name: "SdkUtils", level: nxtpConfig.logLevel });
    ...
    async getRoutersData(params) {
        ...
        const uri = (0, nxtp_utils_1.formatUrl)(this.config.cartographerUrl, "routers_with_balances?", orderIdentifier);
        // Validate uri
    ...
    async getTransfers(params) {
        ...
        const searchIdentifier = userIdentifier +
            routerIdentifier +
            statusIdentifier +
        ...
        const uri = (0, nxtp_utils_1.formatUrl)(this.config.cartographerUrl, "transfers?", searchIdentifier + rangeIdentifier + orderIdentifier + `&${nxtp_utils_1.transfersCastForUrl}`);
        // Validate uri
        (0, helpers_1.validateUri)(uri);
        return await (0, helpers_1.axiosGetRequest)(uri);
    }
     ...
     *
     */
    async checkRouterLiquidity(domainId, asset, topN) {
        const _asset = ethers_1.utils.getAddress(asset);
        const _topN = topN ?? 4;
        const routersByLargestBalance = await this.getRoutersData({ order: { orderBy: "balance", ascOrDesc: "desc" } });
        const eligibleRouters = Array.from(routersByLargestBalance).filter((routerBalance) => routerBalance.domain == domainId && ethers_1.utils.getAddress(routerBalance.local) == _asset);
