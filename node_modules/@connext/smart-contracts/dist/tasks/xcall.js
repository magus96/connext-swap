...
exports.default = (0, config_1.task)("xcall", "Prepare a cross-chain tx")
    ...
    .setAction(async ({ asset: _asset, amount: _amount, connextAddress: _connextAddress, to: _to, callData: _callData, destination: _destination, env: _env, relayerFee: _relayerFee, receiveLocal: _receiveLocal, delegate: _delegate, slippage: _slippage, runs: _runs, accounts: _accounts, showArgs: _showArgs, }, hre) => {
    let tx;
    ...
    console.log("senders: ", senders.map((s) => s.address));
    // Get the origin and destination domains.
    const network = await hre.ethers.provider.getNetwork();
    const originDomain = (0, src_1.chainIdToDomain)(network.chainId);
    ...
    const connext = new ethers_1.Contract(connextAddress, connextDeployment.abi);
    ...
    for (let i = 0; i < senders.length; i++) {
        ...
        if (asset === ethers_1.constants.AddressZero) {
            ...
            const erc20 = await hre.ethers.getContractAt("@matterlabs/zksync-contracts/l1/contracts/common/interfaces/IERC20.sol:IERC20", asset, senders[i]);
            console.log("erc20: ", erc20.address);
            const allowance = await erc20.allowance(senders[i].address, connextAddress);
            console.log("allowance: ", allowance.toString());
            if (allowance.lt(ethers_1.BigNumber.from(amount).mul(runs))) {
                tx = await erc20.approve(connextAddress, ethers_1.constants.MaxUint256);
        ...
        const balanceNeeded = ethers_1.BigNumber.from(amount).mul(runs);
        if (balance.lt(balanceNeeded)) {
            throw new Error(`Account (${senders[i].address}) has balance (${balance.toString()}), which is less than total needed (${balanceNeeded})`);
        }
    }
    ...
    for (let i = 1; i <= runs; i++) {
        const receipts = Promise.all(senders.map(async (sender) => {
