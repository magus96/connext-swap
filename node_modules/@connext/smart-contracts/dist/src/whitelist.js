...
const networks = ["rinkeby", "kovan", "goerli"];
...
const run = async () => {
    ...
    const routerAddresses = routers.map((r) => ethers_1.utils.getAddress(r));
    for (const n of networks) {
        for (const r of routerAddresses) {
            console.log(`Running add router script for router ${r} on network ${n}`);
            const { stdout: out, stderr: err } = await exec(`yarn hardhat setup-router --network ${n} --router ${r} --env ${env}`);
            if (out) {
