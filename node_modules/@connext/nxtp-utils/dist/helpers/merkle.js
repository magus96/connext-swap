...
exports.ZERO_HASH_AT_DEPTH = exports.ZERO_HASHES = exports.SparseMerkleTree = exports.getMerkleProof = exports.MerkleProofVerificationFailed = exports.MerkleRootInvalid = void 0;
const merkletreejs_1 = require("merkletreejs");
const ethers_1 = require("ethers");
const types_1 = require("../types");
const mocks_1 = require("../mocks");
class MerkleRootInvalid extends types_1.NxtpError {
    constructor(context = {}) {
        super("Bad root or branch.", context, MerkleRootInvalid.name);
    }
}
exports.MerkleRootInvalid = MerkleRootInvalid;
class MerkleProofVerificationFailed extends types_1.NxtpError {
    constructor(context = {}) {
        super("Merkle proof verification failed.", context, MerkleProofVerificationFailed.name);
    }
}
exports.MerkleProofVerificationFailed = MerkleProofVerificationFailed;
const getMerkleProof = async (params) => {
    const { leaf, branch } = params;
    const tree = new merkletreejs_1.MerkleTree(branch, ethers_1.utils.keccak256);
...
const ZERO_BYTES = (0, mocks_1.mkBytes32)("0x");
class SparseMerkleTree {
    get maxCount() {
        // For a height of 32, this will be 4,294,967,296 nodes.
        return 2 ** this.height;
    }
    constructor(db, height = 32, _debugLog = false) {
        this.db = db;
        this.height = height;
        this._debugLog = _debugLog;
    }
    ...
    async getRoot() {
        ...
        const zeroHashes = (0, exports.ZERO_HASHES)();
    ...
    verify(index, leaf, proof, expected) {
    ...
    async getProof(index) {
        ...
        const siblings = (0, exports.ZERO_HASHES)();
    ...
    async getSubtreeNodes(depth, pathPrefix) {
        ...
        if (this._debugLog) {
            ...
            console.log("Getting subtree nodes for", pathPrefix[pathPrefix.length - 1] === "0" ? "left" : "right", "path at depth", depth, ": (", lowerBound, "->", upperBound, ") :", t);
        ...
        if (lowerBoundPath.length !== this.height) {
            throw new Error("Lower bound path is incorrect length: " + lowerBoundPath.length.toString() + " != " + this.height.toString());
        }
        return await this.db.getNodes(lowerBound, upperBound);
    }
    // Get root from (sub)array of base layer nodes
    getSubtreeRoot(depth, nodes) {
        ...
        let roots = nodes.concat(new Array(expectedNodeCount - nodes.length).fill(ZERO_BYTES)); //hashedBaseLayer));
        if (this._debugLog) {
...
// Hash function used.
SparseMerkleTree.hash = (a, b) => {
    return ethers_1.utils.solidityKeccak256(["bytes32", "bytes32"], [a, b]);
};
...
const ZERO_HASHES = () => _ZERO_HASHES.slice(0);
exports.ZERO_HASHES = ZERO_HASHES;
const ZERO_HASH_AT_DEPTH = (depth) => _ZERO_HASHES[depth];
exports.ZERO_HASH_AT_DEPTH = ZERO_HASH_AT_DEPTH;
