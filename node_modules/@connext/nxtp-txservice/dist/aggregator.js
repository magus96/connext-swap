...
const DEFAULT_BLOCK_PERIOD = 2000;
// TODO: Multiton?
/**
...
class RpcProviderAggregator {
     ...
     * configuration.
     */
    constructor(logger, domain, config, signer) {
        this.logger = logger;
        this.domain = domain;
        this.config = config;
        ...
        const { requestContext, methodContext } = (0, nxtp_utils_1.createLoggingContext)("ChainRpcProvider.constructor");
        ...
        const filteredConfigs = providerConfigs.filter((config) => {
            const valid = (0, config_1.validateProviderConfig)(config);
            if (!valid) {
        ...
        if (filteredConfigs.length > 0) {
            const hydratedConfigs = filteredConfigs.map((config) => ({
                provider: new shared_1.SyncProvider({
                    url: config.url,
                    user: config.user,
                    password: config.password,
                }, this.domain, config.stallTimeout, this.config.debug_logRpcCalls),
        ...
        if (signer) {
            this.signer =
                typeof signer === "string" ? new ethers_1.Wallet(signer, this.fallbackProvider) : signer.connect(this.fallbackProvider);
        }
        else {
            this.signer = undefined;
        }
        // TODO: Make ttl/btl values below configurable ?
        this.cache = new shared_1.ProviderCache(this.logger, {
            gasPrice: {
                ttl: 30000,
            },
            transactionCount: {
    ...
    async sendTransaction(transaction) {
        ...
        return await this.signer.sendTransaction({
            ...transaction.params,
            value: ethers_1.BigNumber.from(transaction.params.value || 0),
        });
    }
    /**
     ...
     */
    async confirmTransaction(transaction, confirmations = this.config.confirmations, timeout = this.config.confirmationTimeout) {
        ...
        while (!timedOut) {
            ...
            const receipts = (await Promise.all(_receipts)).filter((r) => r !== null && r !== undefined);
            ...
            if (!mined) {
                ...
                if (reverted.length > 0) {
                    throw new shared_1.TransactionReverted(shared_1.TransactionReverted.reasons.CallException, reverted[0]);
                }
        ...
        throw new shared_1.OperationTimeout({
     ...
     * to read from chain.
     */
    async readContract(tx, blockTag = "latest") {
        ...
        return this.execute(false, async (provider) => {
            try {
                ...
                throw new shared_1.TransactionReadError(shared_1.TransactionReadError.reasons.ContractReadError, { error });
            }
        });
    }
    /**
    ...
    async getTransaction(tx) {
        ...
        const errors = [];
        const txs = await Promise.all(tx.responses.map(async (response) => {
            try {
                return this.execute(false, async (provider) => {
    ...
    async estimateGas(transaction) {
        ...
        return this.execute(false, async (provider) => {
            ...
            try {
                return ethers_1.BigNumber.from(result).add(gasLimitInflation ? ethers_1.BigNumber.from(gasLimitInflation) : 0);
            }
            catch (error) {
                throw new shared_1.GasEstimateInvalid(result.toString(), {
                    error: error.message,
                });
            }
        });
     ...
     */
    async getGasPrice(context, useInitialBoost = true) {
        const { requestContext, methodContext } = (0, nxtp_utils_1.createLoggingContext)(this.getGasPrice.name, context);
        ...
        if (hardcoded) {
            ...
            return ethers_1.BigNumber.from(hardcoded);
        }
        ...
        for (let i = 0; i < gasStations.length; i++) {
            ...
            let response;
            try {
                response = await (0, mockable_1.axiosGet)(uri);
                if (response && response.data) {
                    const { fast } = response.data;
                    if (fast) {
                        gasPrice = ethers_1.utils.parseUnits(fast.toString(), "gwei");
                        break;
                    }
                }
        ...
        if (gasPriceMaxIncreaseScalar !== undefined &&
            ...
            if (gasPrice.gt(curbedPrice)) {
                this.logger.debug("Hit the gas price curbed maximum.", requestContext, methodContext, {
                    domain: this.domain,
                    gasPrice: ethers_1.utils.formatUnits(gasPrice, "gwei"),
                    curbedPrice: ethers_1.utils.formatUnits(curbedPrice, "gwei"),
                    gasPriceMaxIncreaseScalar,
                    lastUsedGasPrice: ethers_1.utils.formatUnits(this.lastUsedGasPrice, "gwei"),
                });
        ...
        const min = ethers_1.BigNumber.from(gasPriceMinimum);
        const max = ethers_1.BigNumber.from(gasPriceMaximum);
        ...
        if (gasPrice.lt(min) && this.domain !== 1634886255) {
            ...
            this.logger.warn("Hit the gas price absolute maximum.", requestContext, methodContext, {
                domain: this.domain,
                gasPrice: ethers_1.utils.formatUnits(gasPrice, "gwei"),
                absoluteMax: ethers_1.utils.formatUnits(max, "gwei"),
            });
            gasPrice = max;
            hitMaximum = true;
        }
     ...
     * specified address.
     */
    async getBalance(address, assetId, abi) {
        return this.execute(false, async (provider) => {
            if (assetId === ethers_1.constants.AddressZero) {
                return await provider.getBalance(address);
            }
            else {
                const contract = new ethers_1.Contract(assetId, abi ?? nxtp_utils_1.ERC20Abi, provider);
     ...
     */
    async getDecimalsForAsset(assetId) {
        return this.execute(false, async (provider) => {
            ...
            // Get provider
            const decimals = await new ethers_1.Contract(assetId, nxtp_utils_1.ERC20Abi, provider).decimals();
     ...
     */
    async getBlockTime(blockTag = "latest") {
        return this.execute(false, async (provider) => {
    ...
    async getTransactionCount(blockTag = "latest") {
        ...
        return this.execute(true, async (provider) => {
    ...
    checkSigner() {
        if (!this.signer) {
            throw new nxtp_utils_1.NxtpError("Method requires signer, and no signer was provided.");
        }
    }
    /**
     ...
     */
    async execute(needsSigner, method) {
        ...
        const handleError = (e) => {
            ...
            const error = (0, shared_1.parseError)(e);
            if (error.type === shared_1.ServerError.type || error.type === shared_1.RpcError.type || error.type === shared_1.StallTimeout.type) {
        ...
        if (quorum > 1) {
            ...
            if (filteredResults.length > 0) {
                ...
                if (maxCount < quorum) {
                    ...
                    throw new shared_1.QuorumNotMet(maxCount, quorum, {
                        errors,
                ...
                if (topResponses.length > 0) {
                    ...
                    this.logger.info("Received conflicting top responses from RPC providers.", (0, nxtp_utils_1.createRequestContext)(this.execute.name), undefined, {
                        topResponses,
                        providersCount: this.providers.length,
                        responsesCount: filteredResults.length,
        ...
        throw new shared_1.RpcError(shared_1.RpcError.reasons.FailedToSend, { errors });
    }
    /**
     ...
     */
    async syncProviders() {
        const { requestContext, methodContext } = (0, nxtp_utils_1.createLoggingContext)(this.syncProviders.name);
     ...
     */
    async wait(numBlocks = 1) {
        const pollPeriod = numBlocks * (this.blockPeriod ?? 2000);
        await (0, nxtp_utils_1.delay)(pollPeriod);
    }
    /**
...
exports.RpcProviderAggregator = RpcProviderAggregator;
